...
export class QuestionnaireQualificationService {
  ...
  static getInstance(): QuestionnaireQualificationService {
    if (!QuestionnaireQualificationService.instance) {
      QuestionnaireQualificationService.instance = new QuestionnaireQualificationService()
    }
   ...
   */
  async checkQualification(userId: string, questionnaireId: string, emailVerified: boolean = false): Promise<QualificationStatus> {
    try {
      console.log(`🔍 Checking qualification for user ${userId} and questionnaire ${questionnaireId}`)
      console.log(`📧 Email verification status: ${emailVerified}`)
      
      const qualificationRule = this.QUALIFICATION_RULES.find(rule => rule.id === questionnaireId)
      ...
      const requiresEmailVerification = qualificationRule.requiresEmailVerification || false
      ...
      const completedQuestionnaires = await this.getUserCompletedQuestionnaires(userId)
      ...
      const shouldDisableWhenCompleted = qualificationRule.disableWhenCompleted || false
   ...
   */
  private async getUserCompletedQuestionnaires(userId: string): Promise<string[]> {
    try {
  ...
  hasPrerequisites(questionnaireId: string): boolean {
    const rule = this.QUALIFICATION_RULES.find(rule => rule.id === questionnaireId)
  ...
  addQualificationRule(rule: QuestionnaireQualification): void {
    const existingIndex = this.QUALIFICATION_RULES.findIndex(r => r.id === rule.id)
    if (existingIndex >= 0) {
  ...
  removeQualificationRule(questionnaireId: string): void {
    const index = this.QUALIFICATION_RULES.findIndex(rule => rule.id === questionnaireId)
    if (index >= 0) {
