...
/**
 * Update Questionnaires in Firestore Script (Firebase Admin SDK)
 * 
 * This script uploads the questionnaires to Firestore using Firebase Admin SDK
 * with the corrected route-1.kml ordering from lib/kml-config.ts.
 */
...
console.log("🔄 Starting Questionnaire Upload to Firestore (Admin SDK)...");
...
async function uploadQuestionnairesInFirestore() {
  try {
    ...
    console.log(`✅ Successfully loaded ${questionnaires.length} questionnaires`);
    ...
    const querySnapshot = await db.collection("questionnaires").get();
    ...
    for (const questionnaire of questionnaires) {
      ...
      await db.collection("questionnaires").doc(questionnaire.id).set(docData, { merge: true });
    ...
    console.log(`📊 All questionnaires sourced from lib/kml-config.ts (single source of truth)`);
